...
pytest & 9.0 & Testes do parser LOA e validação automatizada \\
...
\begin{minipage}[t]{2.1cm}\raggedright\path{pjf_loa_funcao}\end{minipage} & LOA PJF & 2026 & Funções orçamentárias (DimLOA, \texttt{Funcao.pdf}) \\
\begin{minipage}[t]{2.1cm}\raggedright\path{pjf_loa_subfuncao}\end{minipage} & LOA PJF & 2026 & Subfunções orçamentárias (DimLOA, \texttt{SubFuncao.pdf}) \\
\hline
\end{tabular*}
\end{table}

Quatro decisões de extração ilustram como o pipeline reage a problemas reais de fonte pública, não a um \textit{dataset} estável de laboratório.
...
A unidade de idempotência é a partição temporal: mês de competência nas planilhas PJF, ano de referência na STN e exercício 2026 na LOA funcional. Reexecutar o pipeline para uma partição deve devolver o mesmo \textit{warehouse}, independentemente de quantas cargas anteriores existam. Três mecanismos sustentam essa propriedade.
...
A revisão de qualidade do \textit{dashboard} (recorte completo, sem filtros de ano ou mês) quantificou a cobertura dos rótulos LOA na página Execução Orçamentária. Entre os 126 pares distintos função/subfunção presentes na execução, 18 (14{,}3\,\%) exibem \textit{fallback} do tipo \texttt{Função XX} ou \texttt{Subfunção XXX} --- isto é, o código aparece na movimentação mensal, mas não foi resolvido contra \texttt{dim\_funcao} ou \texttt{dim\_subfuncao}. Exemplos observados incluem 11/220, 10/310 e 01/031. Os pares de maior volume executado (Saúde --- Atenção Básica, 10/301; Administração --- Administração Geral, 04/122) recebem nomes corretos da LOA; os \textit{fallbacks} concentram-se em combinações de menor pagamento acumulado.
