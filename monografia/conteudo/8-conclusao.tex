...
Cada uma dessas direções é viável com o ferramental atual e não exige reescrita das camadas já implementadas. Trabalho futuro mais ambicioso inclui automação de coleta com agendamento real (hoje as partições suportam \textit{backfill} sob demanda, mas não há \textit{scheduler} ativo; o painel já está em URL pública, porém alimentado pela última materialização manual do banco), consolidação institucional da manutenção do \textit{pipeline} e avaliação empírica de impacto em ações concretas de controle social.
